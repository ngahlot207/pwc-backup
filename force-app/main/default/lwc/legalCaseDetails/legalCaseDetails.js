...
import DEV_LEGAL from '@salesforce/schema/Case.AreThereAnyLegalDeviations__c';
...
const fields =[FI_AGENCY,APPLICANT,PHONE,MOBILE,FI_INITIATE_DATE,BRANCH,REPORT_DATE,DEV_LEGAL,COMPLETE_ADD,BUSINESS_EMP_NAME,NEGATIVE_AREA,PROPERTY_TYPE,
...
export default class LegalCaseDetails extends LightningElement {
  ...
  commentsQueryParams = {
   ...
   get showQryDesc(){
    ...
    connectedCallback(){
        ...
        console.log('this.hasEditAccess:::::504 in legalCaseDetails',this.hasEditAccess);
        // if(this.userRole ==='LHM'){
        //     this.enableForLHM =true;
        // }  
    ...
    @track teamHierParam = {
        ...
        queryCriteria: ''
         ...
         handleChangeDev(event) {
            this.legalDevVal = event.detail.value;
...
@wire(getSobjectData, { params: '$sPDDApprConfig' })
    ...
    levelCompareOfDev() {
        ...
        caseData = {
          ...
          'FinalStatusOfTSR__c','QueryRevert__c','HubManagerReview__c','TAT__c','AreThereAnyLegalDeviations__c'], 
          childObjFields: [],
          queryCriteria: "  "
        };
...
getCaseData(){  
    ...
    try {
        ...
        if (this.records1 && this.records1[index] !== undefined) {
            ...
            this.arrayDeviationInsert=this.records1;
...
handleDelete(event) {
        ...
        if (delRecId) {
           ...
           // this.fetchDeviationData();
        }else if (!delRecId || delRecId === null || delRecId === undefined || delRecId === '') {
            console.error('Invalid deleteRecId:', delRecId);
            this.ShowToastMessage('Error', 'Record Deleted Successfully', 'error', 'sticky');
 ...
 @track btLoanRec = [];
    handleDeleteAction1(event) {
        ...
        if (deleteRecId === undefined) {
            console.log('undefined id delete')
        ...
        // if (deleteRecId.length === 18) 
            ...
            this.handleDeleteRecId1(deleteRecId);
        }

    }

    @track del_recIds = []
    handleDeleteRecId1(delRecId) {
        let fields = {};
        fields.Id = delRecId
        this.del_recIds = []
        this.del_recIds.push(fields)
        console.log("deleteRec_Array ", this.del_recIds);
        deletebtLoanRecord({ rcrds: this.del_recIds }).then((result) => {
            this.showToastMessage('Success', 'Success DOne', 'success', 'sticky');
            this.btLoanRec = [];
            this.fetchDeviationData();
...
///bt copy end
    ...
    if (this.validateForm()) {
        ...
        try {
            ...
            console.log('legal status:'+this.legalDevYesNo);
            console.log('User role :'+this.userRole)
            if(this.legalDevYesNo === 'Y'){
            upsertMultipleRecord({ params: tempArray })
                ...
                console.log('not required create Deviation Rec')
            }
        } catch (error) {
            this.showSpinner = false;
            this.showSpinnerModal = false;
...
handleRemoveRecord(event) {
    console.log('id is delete is out:',this.deleteRecId);
        ...
        else if (!this.deleteRecId || this.deleteRecId === null || this.deleteRecId === undefined || this.deleteRecId === '') {
            console.error('Invalid deleteRecId:', this.deleteRecId);
            this.records.splice(event.target.dataset.index, 1); 
            //this.ShowToastMessage('Error', 'Invalid record ID', 'error', 'sticky');
            this.ShowToastMessage('Success','Record Deleted Successfully','success','dismissable')
           // this.deleteButton=false;
           this.isModalOpen = false;
            this.fetchDeviationData();
            return;
...
handleSave(validate) {
    if (validate) {
        ...
        .then((result) => {
           ...
           console.log('array deviation:',this.arrayDeviationInsert)
...
validateForm() {
    ...
    if(this.status === 'New' || this.status === 'Review Requested' || this.status === 'Query' || this.status === 'Query Resolved' ){
        if((this.reportCount === undefined || this.reportCount < 1  )){
            isValid=false
...
updateRec() {
    ...
    this.caseWrp.AreThereAnyLegalDeviations__c=this.legalDevYesNo;
