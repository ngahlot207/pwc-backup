...
export default class CaptureIncomeDetails extends LightningElement {
    label = {
  ...
  compareByLabel(a, b) {
    ...
    validateForm() {
        let allValid = true; // Start with true, indicating all components are valid
        ...
        listofComponents.forEach(componentType => {
            let component;
    
            switch (componentType) {
                case 'Pension':
                    component = this.template.querySelector("c-capture-pension-income-details");
                    break;
                case 'Rental_woBanking':
                    component = this.template.querySelector("c-capture-rental-income-without-banking");
                    break;
                case 'RentalwBank':
                    component = this.template.querySelector("c-capture-rental-income-details");
                    break;
                case 'CashSal':
                    component = this.template.querySelector("c-capture-salary-income");
                    break;
                case 'Cashflow_Map':
                    component = this.template.querySelector("c-capture-cash-flow-computation-details");
                    break;
                case 'AgriIncome':
                    component = this.template.querySelector("c-capture-agriculture-income-details");
                    break;
                case 'BankCrSal':
                    component = this.template.querySelector("c-capture-bank-credit-salary");
                    break;
                case 'Other_Income':
                    component = this.template.querySelector("c-capture-other-income-details");
                    break;
                case 'Other_Income_Annual':
                    component = this.template.querySelector("c-capture-other-income-annual-details");
                    break;
                case 'Consulting_Income':
                    component = this.template.querySelector("c-capture-consulting-income");
                    break;
            }
    
            // Validate component if found
            if (component) {
                const isComponentValid = component.validateForm();
                if (!isComponentValid) {
                    allValid = false; // Mark the form as invalid if any component validation fails
                }
            }
        });
    
        return allValid; // Return true only if all validations pass, else false
    }
    ...
    @track showLIPVerification = false;
    handleLIPIntitation(){
        this.showLIPVerification = true;
    }
