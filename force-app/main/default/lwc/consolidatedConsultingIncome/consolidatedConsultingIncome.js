...
export default class ConsolidatedConsultingIncome extends LightningElement {
    label = {
    ...
    handleAddrRecordChange(event) {
        ...
        tempParams.queryCriteria = ' where Applicant__r.LoanAppln__c = \'' + this.loanAppId + '\'' + ' AND Applicant__c!=null AND RecordTypeId = \'' + this.recTypeId + '\''
    ...
    Incomeparams = {
        ...
        childObjFields: [],
        queryCriteria: ' where Applicant__r.LoanAppln__c = \'' + this.loanAppId + '\'' + ' AND Applicant__c!=null AND RecordTypeId = \'' + this.recTypeId + '\''
    }
    ...
    handleIncomeResponse(wiredResult) {
        ...
        if (data) {
            ...
            console.log('cosnolidate data:: ' + JSON.stringify(wiredResult));
            this.consolidatedData = this.consolidateData(this.actualIncomeData);
                    console.log('consolidatedArray:: ',this.consolidatedData);
    ...
    consolidateData(records) {
        const monthlyIncomeMap = {};
    
        // Group by Month__c and sum Monthly_Other_Income__c for each month
        if(records){
        records.forEach(record => {
            ...
            monthlyIncomeMap[month].totalIncome += record.Monthly_Other_Income__c;
        });

        this.calculateAverageIncome();
    
        // Convert the map to an array and sort it by Month__c
        return Object.values(monthlyIncomeMap).sort((a, b) => {
            return new Date(`1 ${a.Month__c}`) - new Date(`1 ${b.Month__c}`);
        });
    ...
    calculateAverageIncome() {
        ...
        if (this.consolidatedData.length < 1) {
            this.averageMonthlyOtherIncome = 0;
            console.log('Inside property length check');
        } else {

            for (let i = 0; i < this.consolidatedData.length; i++) {
                let monthlyIncome = 0;
                let value = parseInt(this.consolidatedData[i].totalIncome);
                if (!isNaN(value)) {
                    monthlyIncome = value;
                } else {
                    monthlyIncome = 0;
            ...
            this.averageMonthlyOtherIncome = (totalIncome / this.consolidatedData.length).toFixed(2);
