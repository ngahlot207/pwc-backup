...
export default class ShowCaseDocument extends NavigationMixin(LightningElement) {
    customLabel = {
    ...
    connectedCallback() {
        console.log('caseid in show case document component ', this.caseId);
    ...
    @api handleFilesUploaded() {
        ...
        console.log(" handle file upload called applicantId:", this.caseId, "category : ", this.category, " docType: ", this.type, " subType : ", this.subType);
        fetchId({ applicantId: this.caseId, category: this.category, docType: this.type, subType: this.subType })
            .then((result) => {
                ...
                this.lstAllFiles.forEach(item => {
                    if(item.cDcrtdDate){
                       item.cDcrtdDate = this.dateFormat(item.cDcrtdDate);
                    }
                })
