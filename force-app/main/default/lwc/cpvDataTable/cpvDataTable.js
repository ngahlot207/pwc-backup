...
export default class CpvDataTable extends LightningElement {
  ...
  let  loanDetParams = {
      ...
      ChildObjectRelName: "",
      parentObjFields: ["Id", "Name", "ReqLoanAmt__c","NoCasesNeedToRaised__c","OwnerId","Stage__c",
        "SubStage__c","Product__c","BrchCode__c"],
      childObjFields: [],
    ...
    getSobjectDataNonCacheable({params: loanDetParams}).then((result) => {
       // this.wiredDataCaseQry=result;
        ...
        if (result.parentRecords !== undefined && result.parentRecords.length > 0) {
          ...
          this.prodType = result.parentRecords[0].Product__c;
   ...
   let waiverCpv='Yes'
    ...
    let paramsLoanApp = {
      ...
      parentObjFields: [
        ...
        'IsMandatory__c','Final_Status_of_Field_Verification_by_un__c','Mitigant_for_Change_in_Status__c'
      ],
      queryCriteria:
        ' where  Applicant__c != null AND AccountId !=null AND Loan_Application__c = \'' + this.loanAppId + '\' AND  WaiveCPV__c !=\''+ waiverCpv+'\'  AND RecordType.Name=\''+recordType+ '\' Order by createdDate'
  ...
  @api getBLCaseData() {
    ...
    let waiverCpv='Yes'
     ...
     let paramsLoanApp = {
       ParentObjectName: "Case",
       parentObjFields: [
         "Id", "Phone_No__c","Mobile_No__c", "Address_Type__c", "Address_Line_2__c", "Address_Line_1__c",
         "ApplAddr__c","CaseNumber","AccountId","ContactId", 'IsReinitiated__c', "Applicant__r.TabName__c",
         "Old_Agency__r.Name","Account.Name", "Contact.Name","Status", "ReportResult__c", "CityId__c",
         "Reason_for_reinitiated_FI__c","Reason_for_cancelation__c","Loan_Application__c", "Applicant__c",
         "CreatedDate","ClosedDate",'WaiveCPV__c','RecordTypeId','RecordType.Name','TAT__c','ReviewerComments__c',
         'City__c','ExpiryDate__c','IsReinitiatedExpired__c','IsMandatory__c','OwnerId','Owner.Name','Mitigant_for_Change_in_Status__c',
         'Final_Status_of_Field_Verification_by_un__c'
       ],
       queryCriteria:
         ' where  Applicant__c != null AND Loan_Application__c = \'' + this.loanAppId + '\' AND  WaiveCPV__c !=\''+ waiverCpv+'\'  AND RecordType.Name=\''+recordType+ '\' Order by createdDate'
     ...
     getSobjectData({ params: paramsLoanApp }).then((result) => {
       ...
       try {
         if (result.parentRecords && result.parentRecords.length > 0) {
           ...
           this.BLCaseRecordsData = result.parentRecords;
 
           this.BLCaseRecordsData.forEach(row1 => {
                 const dateTime1 = new Date(row1.CreatedDate);
                 const formattedDate1 = formattedDateTimeWithoutSeconds(dateTime1); 
                 const createdDate1 = `${formattedDate1}`;
                 row1.CreatedDate = createdDate1;
   ...
   get filterCondn(){
    let val;
     val= ' EmpRole__c IN (\''+this.cpaRoles.join('\', \'') + '\') AND BranchCode__c = \'' + this.brchCode + '\' AND Employee__r.IsActive=true AND Employee__c != \''+Id+'\'';
     console.log('val',val);
    return val;
  }
  ...
  @api getWaivedCaseData() {
    ...
    let waiverCpv='Yes'
     ...
     let paramsLoanApp = {
       ...
       queryCriteria:
         ' where  Applicant__c != null AND Loan_Application__c = \'' + this.loanAppId + '\' AND  WaiveCPV__c =\''+ waiverCpv+'\'  AND RecordType.Name=\''+recordType+ '\''
  ...
  getcaseObjDetails(caseId) {
    let obj =this.showBLTable ? this.BLCaseRecordsData.find((item) => item.Id === caseId) : this.caseRecordsData.find((item) => item.Id === caseId);
    ...
    if (obj) {
      ...
      if (this.dataName === "Re-Initiate") {
        ...
        if(this.showBLTable){
          this.newAgencyValue=this.caseSingleRecordData.AccountId ?this.caseSingleRecordData.AccountId :'' ;
          let contactId=this.caseSingleRecordData.ContactId ?this.caseSingleRecordData.ContactId :'' ;
          if(contactId){
            this.fetchUserDetails(contactId);
          }
        }else{
          this.newAgencyValue=this.caseSingleRecordData.AccountId ?this.caseSingleRecordData.AccountId :'' ;
        }
...
get sendToAgency(){
  return this.selectedUser==='Agency'
}

get sendToCPA(){
  return this.selectedUser==='CPA/UW'
}
@track selectedUser;
handleUserChange(event){
this.selectedUser=event.target.value;
if(this.selectedUser==='Agency'){
  this.lookupId='';
}
}
...
@track lookupId;
  handleLookupFieldChange(event) { 
    if (event.detail) {
      console.log('event.detail:::::CPV DATATABLE:713',JSON.stringify(event.detail));
      this.lookUpRec= event.detail.record;
      if(this.lookUpRec){
        this.lookupId = event.detail.record.Employee__c;  
         console.log('lookupId::::::717',this.lookupId);
...
@track conId;
  handleNewAgencyChange(event) {
    ...
    if(filteredData && filteredData.value){
      ...
      this.fetchUserDetails(this.conId);
    }
...
fetchUserDetails(contId) {
   let userDetParam = {
        ParentObjectName: 'User',
        ChildObjectRelName: '',
        parentObjFields: ['Id','ContactId','IsActive'],
        childObjFields: [],
        queryCriteria: ' where ContactId=\''+contId+'\' AND IsActive =true'
        }              
    getSobjectDataNonCacheable({params: userDetParam}).then((result) => {
  
        console.log("CPV Data Table COMP #769>>>>>", result);
        if (result.parentRecords !== undefined && result.parentRecords.length > 0) {
          this.portalUserId = result.parentRecords[0].Id;
        
        }
      })
      .catch((error) => {
        console.log("Error in CPV Data Table COMP #776", error);
      });
  }
  closeModal() {
    ...
    if (this.caseSingleRecordData) {
      ...
      if (this.yesLabel === "Re-Iniate") {
        ...
        fields["Reason_for_reinitiated_FI__c"] = this.textAreaValue
          ? this.textAreaValue
          : "";
          if(this.showBLTable && this.selectedUser === 'CPA/UW'){
            fields.OwnerId = this.lookupId;
          }else{
          ...
          fields.OwnerId = this.portalUserId;
          }
        ...
        fields["IsMandatory__c"] = this.caseSingleRecordData.IsMandatory__c; //LAK-7287
...
if(obj){   
console.log('Comments Records create ##804', obj); 

      upsertMultipleRecord({ params: obj })
      .then(result => {     
            console.log('Comments Records create ##808', result);
          ...
          .catch(error => {
          this.showSpinner = false;
          console.log("CASE COMMENTS RESULT ERRO:::::816", error);
          })
          }
}
...
validateForm() {
  ...
  if (!this.checkValidityLookup()) {
    console.log("IN IF lookup  #1006 in cpv datatable>>>");
    isValid = false;
}
