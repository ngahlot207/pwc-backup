...
//Custom labels
import OtherIncomeAnnual_Save_SuccessMessage from '@salesforce/label/c.OtherIncomeAnnual_Save_SuccessMessage';
import OtherIncome_ReqFields_ErrorMessage from '@salesforce/label/c.OtherIncome_ReqFields_ErrorMessage';
import OtherIncome_3monthsOtherIncome_ErrorMessage from '@salesforce/label/c.OtherIncome_3monthsOtherIncome_ErrorMessage';
...
import Other_IncomeAnnual_Details_Deleted_Success from '@salesforce/label/c.Other_IncomeAnnual_Details_Deleted_Success';


export default class CaptureOtherIncomeAnnualDetails extends LightningElement {
    label = {
        OtherIncomeAnnual_Save_SuccessMessage,
        OtherIncome_ReqFields_ErrorMessage,
        OtherIncome_3monthsOtherIncome_ErrorMessage,
        RentalIncWithoutBank_FileAccptdate_ErrorMessage,
        Other_IncomeAnnual_Details_Deleted_Success
    }
    ...
    @track averageMonthlyLessTax;
    @track averageMonthlyAftertax;
    @track properties = [];
    @track disableMode;
    ...
    Incomeparams = {
        ...
        ChildObjectRelName: '',
        parentObjFields: ['Id', 'Index__c', 'Month__c', 'Monthly_Other_Income__c', 'Income_after_tax__c', 'Less_Tax__c', 'Applicant__r.Average_Other_Income__c'],
        childObjFields: [],
    ...
    getCurrentandLastYear(){
        ...
        const currentFY = `FY ${financialYearStart}-${financialYearStart + 1}`;
        const lastFY = `FY ${financialYearStart - 1}-${financialYearStart}`;

        this.financialYearOptions = [
        { label: currentFY, value: currentFY },
        { label: lastFY, value: lastFY }
        ];
        console.log('this.financialYearOptions ',this.financialYearOptions)
    }
        ...
        if (this.actualIncomeData && this.actualIncomeData.length > 0) {
            ...
            this.actualIncomeData.forEach(newItem => {
                let newObj = {
                    ...
                    Id: newItem.Id,
                    Income_after_tax__c: newItem.Income_after_tax__c ? newItem.Income_after_tax__c : '',
                    ...
                    Less_Tax__c: newItem.Less_Tax__c ? newItem.Less_Tax__c.toString() : '',
                    isDisabled : false
                };
                ...
                this.calculateAvgLessTaxIncome();
                this.calculateAvgAfterTaxIncome();
    ...
    handleDeleteAction(event) {
        ...
        //         .then(() => {
        //             this.showToastMessage("Success", this.label.OtherIncomeAnnual_Save_SuccessMessage, "success", "dismissible");
        ...
        if (this.recordDelId) {
            ...
            .then(result => {
               ...
               this.showToastMessage("Success", this.label.Other_IncomeAnnual_Details_Deleted_Success, "success", "dismissible");
               this.showDeleteConfirmation = false;
               refreshApex(this.wiredData);
            })
            .catch(error => {
    ...
    inputChangeHandler(event) {
        ...
        if (event.target.dataset.fieldname == "Less_Tax__c") {
            this.properties[event.target.dataset.fieldname] = event.target.value;
            this.properties[event.target.accessKey - 1].Less_Tax__c = event.target.value;
            this.calculateAvgLessTaxIncome();

        }

        if (event.target.dataset.fieldname == "Income_Head1__c") {
            this.properties[event.target.dataset.fieldname] = event.target.value;
            this.properties[event.target.accessKey - 1].Income_Head1__c = event.target.value;

            if (event.target.dataset.fieldtype === 'string') {
                let strVal = event.target.value;
                this.properties[event.target.accessKey - 1].Income_Head1__c = strVal.toUpperCase();
            }
        }

        if (event.target.dataset.fieldname == "Income_after_tax__c") {
            this.properties[event.target.accessKey - 1].Income_after_tax__c = event.target.value;
            this.calculateAvgAfterTaxIncome();
        }
    ...
    calculateAverageIncome() {
        ...
        if (this.properties.length < 1) {
            ...
            this.averageMonthlyOtherIncome = (totalIncome / 12).toFixed(2);
    ...
    calculateAvgLessTaxIncome() {
        console.log('Inside calculateAvgLessTaxIncome');
        let totalIncome = 0;
        this.averageMonthlyLessTax = 0;
        if (this.properties.length < 1) {
            this.averageMonthlyLessTax = 0;
            console.log('Inside property length check');
        } else {
            ...
            for (let i = 0; i < this.properties.length; i++) {
                let monthlyIncome = 0;
                let value = parseInt(this.properties[i].Less_Tax__c);
                if (!isNaN(value)) {
                    monthlyIncome = value;
                } else {
                    monthlyIncome = 0;
            ...
            this.averageMonthlyLessTax = (totalIncome / 12).toFixed(2);
            console.log('In else block average monthly icome', this.averageMonthlyLessTax);
        }
    ...
    calculateAvgAfterTaxIncome() {
        console.log('Inside calculateAverageIncome');
        let totalIncome = 0;
        this.averageMonthlyAftertax = 0;
        if (this.properties.length < 1) {
            this.averageMonthlyAftertax = 0;
            console.log('Inside property length check');
        } else {
            ...
            for (let i = 0; i < this.properties.length; i++) {
                let monthlyIncome = 0;
                let value = parseInt(this.properties[i].Income_after_tax__c);
                if (!isNaN(value)) {
                    monthlyIncome = value;
                } else {
                    monthlyIncome = 0;
            ...
            this.averageMonthlyAftertax = (totalIncome / 12).toFixed(2);
            console.log('In else block average monthly icome', this.averageMonthlyAftertax);
        }
    ...
    formIntialCashSalaryData(month1Options) {
        ...
        for (let i = 0; i < 1; i++) {
            ...
            dataRow['Less_Tax__c'] = '';
            dataRow['Income_Head1__c'] = '';
            dataRow['Income_after_tax__c'] = '';
    ...
    @api validateForm() {
        ...
        //     isValid = false;
        //     this.showToastMessage("Error", this.label.OtherIncome_3monthsOtherIncome_ErrorMessage, "error", "dismissible");
        // } else {
    ...
    @api handleUpsert() {
        if (this.properties.length > 0 && this.isSelected === true) {
            ...
            this.showToastMessage("Error", 'Other Income Details Not Available to Save', "error", "dismissible");
            this.showSpinner = false;
        }
    }
