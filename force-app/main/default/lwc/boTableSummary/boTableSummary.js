...
export default class BoTableSummary extends LightningElement {
    ...
    getAppRelDetails() {
        ...
        const params = {
            ...
            parentObjFields: [
                ...
                'Loan_Applicant__c', 'Related_Person__c', 'Related_Person__r.Id','Related_Person__r.MobNumber__c', 'Related_Person__r.LoanAppln__c', 'Related_Person__r.UCID__c',
                 'Related_Person__r.Gender__c','Related_Person__r.Age__c', 'Shareholding__c', 'BeneficialOwner__c', 'Related_Person__r.Relationship__c'
            ],
            queryCriteria: ' where Loan_Applicant__c = \'' + this._appId + '\' AND Relationship_Type__c  IN (\'' + arra.join('\', \'') + '\') AND BeneficialOwner__c = \'' + yesLabel + '\''
                ...
                if (result.parentRecords && result.parentRecords.length > 0) {
                    ...
                    result.parentRecords.forEach((item) => {
                        ...
                        if (item.Related_Person__r) {
                            ...
                            obj.Gender__c = item.Related_Person__r.Gender__c ? item.Related_Person__r.Gender__c : '';
                            obj.Age__c = item.Related_Person__r.Age__c ? item.Related_Person__r.Age__c : '';
                            obj.Relationship__c = item.Related_Person__r.Relationship__c  ? item.Related_Person__r.Relationship__c : '';
                            obj.Shareholding__c = item.Shareholding__c ? item.Shareholding__c : '';
                            obj.BeneficialOwner__c = item.BeneficialOwner__c ? item.BeneficialOwner__c : '';
                        }
