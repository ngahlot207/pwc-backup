...
import VERIFICATION_INITIATION from "@salesforce/schema/SchMapping__c.Verification_Initiation__c";
import SCHEME_RECORD_ID from "@salesforce/schema/SchMapping__c.Id";
...
export default class VettingDetails extends LightningElement {
...
//old
  /*get disableInitiateTSR(){
    return this.disableMode || this.disableInit || this.isLegalStatus;
  }*/
  get disableInitiateTSR(){
    return this.disableMode || this.disableInit || this.closedCaseDisableTsr || this.isLegalStatus || this.isLegalWaivedStatus;
  }
...
@wire(getPicklistValuesByRecordType, {
  ...
  ApplAssetPicklistHandler({ data, error }) {
    if (data) {
      ...
      this.tsrOptions=[...this.generatePicklist(data.picklistFieldValues.IsVettingFiredToSameLegalVendor__c)];
    }
    if (error) {
  ...
  let loanDetParams = {
      ...
      ChildObjectRelName: "",
      parentObjFields: ["Id", "Name", "ReqLoanAmt__c","OwnerId","Stage__c","SubStage__c","SchemeId__c"],
      childObjFields: [],
    ...
    getSobjectDataNonCacheable({params: loanDetParams}).then((result) => {
       // this.wiredDataCaseQry=result;
        ...
        if (result.parentRecords !== undefined && result.parentRecords.length > 0) {
          ...
          this.lanSchemeId=result.parentRecords[0].SchemeId__c;
          if(this.lanSchemeId!=undefined || this.lanSchemeId!=null){
            this.fetchScheme();
          }

          this.getCaseData();
          this.getWaivedCaseData();
          if((this.currentUserId===this.lanOwner) && ((this.lanStage==='Disbursed' && (this.lanSubStage==='Additional Processing' ))) ){
  ...
  @track _lanSchemeId;
  @api get lanSchemeId() {
    return this._lanSchemeId;
  }
  set lanSchemeId(value) {
    this._lanSchemeId = value;
    this.setAttribute("lanSchemeId", value);
    //this.handleLoanRecordIdChange();
  }

  @track schemeDesc;
  @track schemeId;
  @track isInitiatePostdisbursement=false;
  fetchScheme() {
    let SchemeMappingParams = {
        ParentObjectName: "SchMapping__c",
        ChildObjectRelName: "",
        parentObjFields: ["Id", "SchmId__c", "SchemeDesc__c"],
        childObjFields: [],
        queryCriteria: " where Id = '" + this._lanSchemeId + "' limit 1"
      };
      getSobjectDataNonCacheable({params: SchemeMappingParams}).then((result) => {
         // this.wiredDataCaseQry=result;
          console.log("result scheme mapping", result);
          if (result.parentRecords !== undefined && result.parentRecords.length > 0) {
            this.schemeId = result.parentRecords[0].SchmId__c;
            this.schemeDesc = result.parentRecords[0].SchemeDesc__c;
           
            if(this.schemeId!=undefined || this.schemeId!=null){
              const postDisburseSchemeIds = [545, 533, 534, 537, 538, 612, 613, 608, 609, 611, 610, 557, 561, 565, 566, 562];
              if (postDisburseSchemeIds.includes(Number(this.schemeId))) {
                console.log("schemeid is present");
                this.updateSchemeVerification();
                this.isInitiatePostdisbursement=true;
            } else {
                console.log("schemeid is not present");
            }
        ...
        .catch((error) => {
          console.log("scheme DETAILS #731", error);
        });
    }

  updateSchemeVerification(){
    let comtArr=[];
    const fields = {};
    fields[VERIFICATION_INITIATION.fieldApiName] = "Vetting Post Disbursement";
    fields[SCHEME_RECORD_ID.fieldApiName] = this._lanSchemeId;    
    fields.sobjectType = "SchMapping__c";
    comtArr.push(fields);
    if(comtArr && comtArr.length>0){
      //this.upsertCaseComment(comtArr);      
        
        upsertMultipleRecord({ params: comtArr })
        .then(result => {     
        ...
        .catch(error => {
        this.showSpinner = false;
        console.error('error in scheme verification update', error)
        })
  ...
  fetchApplAsset() {
   let assetParams = {
      ...
      'TSRECForNoYrs__c','IsTSRFiredToSameLegalVendor__c','IsVettingFiredToSameLegalVendor__c' ],
      childObjFields: [],
...
if(obj){   
console.log('Comments Records create Vetting  ##1025', obj); 

upsertMultipleRecord({ params: obj })
.then(result => {     
  console.log('Comments Records create Vetting ##1029', result);
  let oldFields={};
...
.catch(error => {
this.showSpinner = false;
console.error('Line no VETTING DETAILS ##1043', error)
})
}
}
...
// this might not be using need to check later
  ...
  get vettingInit(){
    return this.popUpHeader === 'Inititate Vetting Case' ||  this.popUpHeader === 'Re-Inititate Vetting Case';
  }
...
@track remarkLabel;
  handleInitiate(event) {
    ...
    //lak-9389
    if(this.isInitiatePostdisbursement==true && this.lanStage!="Disbursed"){
      this.showToastMessage('Info', 'Vetting can be initiated in Post Disbursement', 'info', 'sticky');
      return;
    }
    this.showSpinner=true;
    ...
    if(this.initlabel === 'Initiate Vetting'){
      this.checkInitiate=true;
      this.waiveModal=false;
      this.popUpHeader = 'Inititate Vetting Case'
      this.remarkLabel='Remarks for Vetting Agency';
       this.docList=[]
       this.initiateTSR=true;
      
    }
  
    if(this.initlabel === 'Waive Vetting'){
      this.checkInitiate=false; 
      this.waiveModal=false;
      this.popUpHeader = 'Waive Vetting Case'
    ...
    if(this.initlabel === 'Re-Intiate'){
      this.checkInitiate=true;
      this.waiveModal=false;
       this.popUpHeader = 'Re-Inititate Vetting Case'
      this.caseRecordId = event.target.dataset.caseid;
...
@track caseReinitiated=false;
  ...
  handleInputChange(event) {
    ...
    if (event.target.label === "Is Vetting fired to same legal Vendor") {
    ...
    if (event.target.label === "Remarks for Vetting Agency") {
  ...
  createCase() {
    if (this.validateForm()) {
      ...
      try {
        ...
        caseObj.Mobile_No__c=this.appMob;
        ...
        caseObj.CityId__c = this.assetCityId;
          ...
          if(this.initlabel==='Initiate Vetting' || this.initlabel==='Re-Intiate'){
            if(this.accId1){
...
updateApplicantAsset(){
 ...
 // fields.IsVettingFiredToSameLegalVendor__c=this.isVettingSameVendor;
  fields.TSRECForNoYrs__c=this.yrType;

  if(fields){
    this.upsertApplAssetData(fields);
    console.log('::::::::1530::::::::',fields);
  }
...
upsertApplAssetData(obj){
  ...
  .catch(error => {
    this.showSpinner = false;
    console.error('Line no APPLICANT ASSET IN Vetting DETAILS ##1514', error)
  })
}
}
  ...
  validateForm() {
    ...
    //    console.log('inside check combobox' +element);
  ...
  createCaseDocument() {
    ...
    console.log('caseDocDtl:::::1766',caseDocDtl);
  ...
  upsertDataCaseDoc(obj){
    if(obj){   
    console.log('Case Document IN Vetting Detail Records create ##1773', obj); 
    
    upsertMultipleRecord({ params: obj })
    .then(result => {     
        console.log('Case Document Detail Records insert for Doc Dtl Vetting Details ##1777', result);
        this.showToastMessage('Success', this.label.Technical_Doc_SuccessMessage, 'success', 'sticky');
        this.showSpinner=false;
    ...
    .catch(error => {
      this.showSpinner = false;
      console.error('Line no Vetting DETAILS ##1784', error)
    })
  }
  }
...
//legal negative or positive

handleLoanRecordIdChange() {
  let tempParams = this.pramForCaseLegal;
  tempParams.queryCriteria = ' where Loan_Application__c = \''+ this._loanAppId + '\' AND RecordType.name = \'Legal\' order by CreatedDate desc';
...
@track pramForCaseLegal={
  ParentObjectName:'Case',
  ChildObjectRelName:'',
  parentObjFields:[  'Id','WaiveCPV__c','FinalStatusTSRReportFromHLM__c','ApplAssetId__r.CityId__c','ReportResult__c','WaiverReason__c','AllPropertyOwnersArePartOfLoanStru__c','IsTheTitleClearNdMarketable__c','Remarks_regarding_the_case__c','Date_of_Report__c','AccountId','Account.Name','DateofInitiation__c'],
  childObjFields:[ ],
  queryCriteria: ' where Loan_Application__c = \''+ this._loanAppId + '\' AND RecordType.name = \'Legal\' order by CreatedDate desc'
}

_wireForLeaglRecType
@track parentRecForLegal;
@track isLegalStatus=true;
@track isLegalWaivedStatus=false;
    LegalStatus() {
      getSobjectDataNonCacheable({params: this.pramForCaseLegal}).then((result) => {
          if (result.parentRecords !== undefined && result.parentRecords.length > 0) {
            ...
            if(typeof this.parentRecForLegal.FinalStatusTSRReportFromHLM__c !== 'undefined'){
              if(this.parentRecForLegal.FinalStatusTSRReportFromHLM__c=='Positive' || this.parentRecForLegal.FinalStatusTSRReportFromHLM__c=='Refer'){
         ...
         if(typeof this.parentRecForLegal.WaiveCPV__c !== 'undefined'){
          if(this.parentRecForLegal.WaiveCPV__c=='Yes'){
            this.isLegalWaivedStatus=true;
          }
          else{
            this.isLegalWaivedStatus=false;
          }
     }
