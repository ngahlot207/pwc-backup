...
export default class LipVerificationDataTable extends LightningElement {
  ...
  @api getCaseData() {
   let recordType='LIP Vendor case';
    ...
    let paramsLoanApp = {
      ...
      queryCriteria:
        ' where  Applicant__c != null AND Applicant__c = \'' + this._applicantIdOnTabset + '\' AND RecordType.Name=\''+recordType+ '\' Order by createdDate'
    ...
    getSobjectData({ params: paramsLoanApp }).then((result) => {
      ...
      try {
        if (result.parentRecords && result.parentRecords.length > 0) {
          ...
          if( this.lanStage !== 'UnderWriting' && this.lanStage !== 'DDE'){
            item.isReinitDis = true
           }
  ...
  @api getWaivedCaseData() {
    ...
    let waiverCpv='Yes'
     ...
     let paramsLoanApp = {
       ...
       queryCriteria:
         ' where  Applicant__c != null AND Applicant__c = \'' + this._applicantIdOnTabset + '\'  AND RecordType.Name=\''+recordType+ '\''
