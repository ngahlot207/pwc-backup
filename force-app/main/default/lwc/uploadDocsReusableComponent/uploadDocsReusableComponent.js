...
export default class extends NavigationMixin(LightningElement) {
        ...
        if (isMixFiles) {
            ...
            for (var i = 0; i < tempFileArray.length; i++) {
                ...
                if (extensionName != '.jpg' && extensionName != '.jpeg' && tempFileArray[i].size > this.maxFileSize) {
                    let maxSizeNew = this.maxFileSize / (1024 * 1024);
                    this.showToast(
                        "Error!",
                        "error",
                        "Maximum File Size should be " + maxSizeNew + "Mb.",
                        "sticky"
                    );
                    //this.files = [];
                    return;
    ...
    createDocumentDetailRecord() {
        console.log('this. LIP Document', this.applicantId, this.loanAppId, this.docCategory, this.docType, this.docName)
        createAppkycDd({ applicantId: this.applicantId, loanAppId: this.loanAppId, docCategory: this.docCategory, docType: this.docType, docSubType: this.docName })
            .then((result) => {
                console.log('DocumantDetail And AppKyc  Created   ', JSON.stringify(result));
        ...
        this.template.querySelector("iframe").contentWindow.postMessage(
            JSON.parse(
                JSON.stringify({
                    source: "lwc",
                    files: this.fileData,
                    parameters: this.documentDetaiId,//'a08C4000007WKIwIAO'
...
// LAK-9819
  @api clearFiles(){
        if(this.clearFileList){
            console.log('this.clearFileList::::',this.clearFileList);
